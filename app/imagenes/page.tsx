...
export default async function ImagenesPage() {
  ...
  const { data: books, error } = await supabase
    .from('books')
    .select('id, title, author, isbn, original_front_image_url, publish_front_image_url, publish_back_image_url, link_amazon, link_gandhi, link_buscalibre, link_sotano, link_pendulo, user_id')
    .eq('status_code', 5)
